...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key: Tensor, query: Tensor, value: Tensor, attention_mask: Tensor):
    ...
    att = (query @ key.transpose(-2, -1)) / torch.sqrt(torch.Tensor([self.attention_head_size], device=query.device)) # (b, h, n, n)
    # attention_mask (b, 1, 1, n)
    bs, _, seq_len, _ = att.size()
    att = att + attention_mask
    att = F.softmax(att, dim=-1)
    y = att @ value # (b, h, n, n) @ (b, h, n, d/h) = (b, h, n, d/h)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    input: the input of the previous layer (b, n, d)
    output: the output of the previous layer (b, n, d)
    dense_layer: used to transform the output (d -> d)
    dropout: the dropout to be applied 
    ln_layer: the layer norm to be applied (d -> d)
    """
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    inputs_embeds = self.word_embedding(input_ids) # (b, n, d)
    ...
    emb = inputs_embeds + pos_embeds + tk_type_embeds
  ...
  def forward(self, input_ids, attention_mask):
    ...
    embedding_output = self.embed(input_ids=input_ids) # (b, n, d)

    # feed to a transformer (a stack of BertLayers)
    sequence_output = self.encode(embedding_output, attention_mask=attention_mask)

    # get cls token hidden state
    first_tk = sequence_output[:, 0] # (b, 1, d)
